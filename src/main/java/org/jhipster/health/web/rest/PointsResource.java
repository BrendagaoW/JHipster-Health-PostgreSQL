...
import org.jhipster.health.domain.PointsPerWeek;
...
public class PointsResource {
    ...
    public ResponseEntity<List<Points>> getAllPoints(Pageable pageable)
        ...
        if (SecurityUtils.isCurrentUserInRole(AuthoritiesConstants.ADMIN)){
            ...
            log.debug("REST request to get a page of Points for user with admin role");
        } else {
            page = pointsRepository.findByUserIsCurrentUser(pageable);
            log.debug("REST request to get a page of Points for user non-admin");
        }
    ...
    public ResponseEntity<PointsPerWeek> getPointsThisWeek()
        ...
        LocalDate startOfWeek = now.minusDays(now.getDayOfWeek().getValue());
